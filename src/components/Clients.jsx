...
export default function Clients({ setNotification }) {
  ...
  const handleToggleStatus = async (client) => {
    const newStatus = client.status === 'Active' ? 'Hold' : 'Active';
    const payload = {
      ...client,
      status: newStatus
    };
    try {
      const data = await safeFetchJson(`/api/clients/${client.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (data.success) {
        setNotification({ message: `Client status changed to ${newStatus}`, type: 'success' });
        fetchClients();
      } else {
        setNotification({ message: data.error || 'Failed to update status', type: 'error' });
      }
    } catch (err) {
      setNotification({ message: err.message, type: 'error' });
    }
                  ...
                  <tr key={c.id}>
                    ...
                    <td>
                      <div className="actions-row" style={{ justifyContent: 'flex-end' }}>
                        <button className="icon-btn" title={c.status === 'Active' ? "Pause Outreach" : "Resume Outreach"} onClick={() => handleToggleStatus(c)}>
                          {c.status === 'Active' ? (
